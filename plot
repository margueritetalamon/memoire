...
plot_hist_densite <- function(melange, ech_gibbs, n_gibbs, sequence, bn){
  ...
  lines(sequence,dmixtmod(sequence,c(mean(ech_gibbs$p[bn:n_gibbs,1]),mean(ech_gibbs$p[bn:n_gibbs,2])),c(mean(ech_gibbs$mu[bn:n_gibbs,1]),mean(ech_gibbs$mu[bn:n_gibbs,2])),c(mean(ech_gibbs$sig[bn:n_gibbs,1]),mean(ech_gibbs$sig[bn:n_gibbs,2]))),lwd=3,col='steelblue')
}



plot_hist_mu_marg <- function(sim_gibbs){
  hist(sim_gibbs$mu[,1],breaks=20,freq=FALSE,main='',xlab='',col='lightcyan3',ylim=c(0,2),xlim=c(0,4))
  hist(sim_gibbs$mu[,2],breaks=20,freq=FALSE,main='',xlab='',col='darkseagreen3',ylim=c(0,2),xlim=c(0,4))  
}
